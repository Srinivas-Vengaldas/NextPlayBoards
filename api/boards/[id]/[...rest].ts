...
/**
 * Explicit dynamic sub-route for `/api/boards/:id/*` (labels, members, team-members, etc).
 * Delegates to the root catch-all so auth/RLS logic remains centralized.
 */
export default async function boardIdSubRoute(req: VercelRequest, res: VercelResponse) {
  ...
  req.url = `/api/boards/${encodeURIComponent(id)}${restPath}${search}`;
